...
export function FloatingChatbot({ onSelectProgram }) {
  ...
  if (!isOpen) {
    return (
      <button type="button" className="quickbox-ai" onClick={() => setIsOpen(true)}>
  ...
  return (
    ...
    >
      ...
      {/* ProgramChat.Root를 항상 마운트하여 상태 유지 */}
      <ProgramChat.Root>
        <div className={`ai-chatbot-body ${isMinimized ? "hidden" : ""}`}>
          ...
          <ProgramChat.Messages
            ...
            renderStreaming={(content) => (
              <MessageBubble content={content} isUser={false} isStreaming onScroll={scrollToBottom} />
            )}
