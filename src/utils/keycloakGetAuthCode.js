...
const resolveCurrentMemberNo = () => {
  return String(useAuthStore.getState().user?.id ?? '').trim();
};

// 로그인후 원패스 가입 유도시
export function onePassJoin() {
  // 가입 유도 플로우는 현재 우리 사이트에 로그인된 회원만 타므로,
  // 상대가 요구하는 mbrId 는 SP 측 회원 ID인 우리 mbr_no(authStore.user.id)를 전달한다.
  const memberNo = resolveCurrentMemberNo();
  if (!memberNo) {
  ...
  // 현재는 상대가 요구하는 SP 회원번호만 mbrId로 전달하고 state 검증은 사용하지 않는다.
