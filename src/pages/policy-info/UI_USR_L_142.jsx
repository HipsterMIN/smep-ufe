...
const UI_USR_L_142 = () => {
    ...
    const routeBbsNo = useMemo(() => {
        const currentMatch = matches[matches.length - 1];
        return [...matches]
            .reverse()
            .find((match) => match?.handle?.bbsNo != null)?.handle?.bbsNo ?? currentMatch?.handle?.bbsNo;
    }, [matches]);
    const queryBbsNo = useMemo(() => String(searchParams.get('bbsNo') ?? '').trim(), [searchParams]);
    const bbsNo = useMemo(
        () => queryBbsNo || String(routeBbsNo ?? '').trim(),
        [queryBbsNo, routeBbsNo],
    );
    ...
    const boardTitle = useMemo(
        () => depth1Menu?.menuNm || postDetail?.bbsNm || '',
        [depth1Menu, postDetail],
    );
    const postTitle = useMemo(() => postDetail?.pstTtl || '-', [postDetail]);
    const pstSrcCn = useMemo(
        () => String(postDetail?.pstSrcCn ?? postDetail?.pstSrcCn ?? '').trim() || '-',
        [postDetail],
    );
    ...
    const attachFiles = useMemo(() => {
        if (Array.isArray(postDetail?.attachFiles)) return postDetail.attachFiles;
        if (Array.isArray(postDetail?.atchFiles)) return postDetail.atchFiles;
        return [];
    }, [postDetail]);
    ...
    const buildPostLink = (targetPstNo) => {
        ...
        const search = bbsNo ? `?bbsNo=${encodeURIComponent(bbsNo)}` : '';
        return `../${targetPstNo}${search}`;
    };

    const handleNavigationClick = (event, targetPstNo) => {
        if (targetPstNo == null || !bbsNo) {
