...
const OnePassSsoCallback = () => {
  ...
  useEffect(() => {
    // 첫 진입 로그다. 실제 /sso 콜백 진입 여부, search 존재 여부, StrictMode 재실행 여부를 가장 먼저 확인할 때 본다.
    ...
    if (hasHandledRef.current) {
      // 같은 브라우저 진입에서 effect 가 다시 돈 경우를 구분하는 로그다.
      // React StrictMode 재실행인지, 예상치 못한 remount 인지 볼 때 reason 과 hasHandled 흐름을 같이 본다.
    ...
    if (!state || !savedState || state !== savedState) {
      ...
      // 여기선 missingState, missingSavedState, mismatchedState 셋 중 무엇이 true 인지를 먼저 보면 된다.
