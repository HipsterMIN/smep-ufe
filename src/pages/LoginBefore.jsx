...
import arrowForwardIcon from '../assets/sub/ico_arrow_forward_ios.svg';
...
const LoginBefore = () => {
  ...
  const handleServiceLogin = (event) => {
    event?.preventDefault();
    // 왜 필요한지: 오른쪽 일반 로그인 영역은 기존 ID/PW 로그인 화면으로 보내야 기존 로컬 로그인 기능을 변경하지 않는다.
  ...
  return (
    <div className="contents login-before-contents">
      ...
      <style>{`
        ...
        .login-before-contents .login-home-container .login-img-box {margin: 0; width: clamp(100px, 27vw, 150px); aspect-ratio: 1 / 1; border-radius: 50%; background: #EDF3FC; display: flex; align-items: center; justify-content: center;}
        .login-before-contents .login-home-container .login-img-box img {vertical-align: bottom; width: 60%; height: 60%; object-fit: contain;}
        .login-before-contents .login-home-container .login-left-box .login-img-box img {max-width: 150px;}
        .login-before-contents .login-home-container .login-right-box .login-img-box img {max-height: 130px;}
        ...
        .login-before-contents .login-home-container .login-btn .icon {display: block; width: clamp(12px, 3vw, 16px); height: clamp(12px, 3vw, 16px); background: url(${arrowForwardIcon}) center / contain no-repeat;}
        .login-before-contents .login-home-container .login-btn.white .icon {filter: brightness(0) saturate(100%) invert(19%) sepia(79%) saturate(1964%) hue-rotate(211deg) brightness(89%) contrast(97%);}
      ...
      <div className="login-home-container">
        <div className="login-left-box">
          ...
          <p className="login-box-txt">중기 통합회원으로 로그인하시면 <br />다양한 유관시스템의 서비스를 이용하실 수 있습니다.</p>
          <a href="javascript:;" className="login-btn" onClick={handleOnePassLogin}>
            <span>로그인 하기</span>
            <i className="icon ico-arrow-forward-ios" aria-hidden="true"></i>
          </a>
        ...
        <div className="login-right-box">
          ...
          <p className="login-box-txt">일반 회원으로 로그인하시면 <br />해당 서비스를 이용하실 수 있습니다</p>
