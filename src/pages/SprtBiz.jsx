...
const SprtBiz = () => {
  ...
  const search = async (pageParam) => {
    const response = await apiClient.get(`/api/v1/sprtBiz?page=${pageParam}&searchText=${searchText}&searchType=${searchType}`);
            ...
            <section className={`tab-conts ${activeTabIndex === 0 ? 'active' : ''}`}>
              ...
              <div className="search-top-box">
                <div className="sch-form-wrap" ref={schFormWrapRef1}>
                  ...
                  <div className="sch-input">
                    <input type="text" className="krds-input" placeholder="공고명·사업명·기관명으로 검색하세요"
                      title="검색어 입력"
                      value={searchText}
                  ...
                  <button type="button" className="krds-btn medium text" onClick={() => handleToggleFilter(0)}><i
              ...
              <div className="search-list-top">
                <ul className="sch-info" aria-live="polite">
                  <li>검색 결과 <span className="point">0</span>개</li>
                </ul>
                ...
                {items.map((item, index) => (
                  <li className="structured-item" key={index}>
                    <div className="card-top">
                      <span className="krds-badge bg-light-primary">{item.bizpbancclsfcd}</span>
                    </div>
              ...
              <ul className="krds-structured-list">
                {items.map((item, index) => (
                  <li className="structured-item" key={index}>
                    ...
                    <div className="card-body">
                      <a href="#" className="c-text">
                        <p className="c-tit no-icon"><span
                          className="span onellipsis-2">2026년 소공인 복합지원센터 구축ㆍ운영사업 본공모</span></p>
                        <p className="c-txt onellipsis-2">
                            소공인 집적지 활성화와 혁신성장 기반 조성을 위한「2026년 소공인 복합지원센터 구축ㆍ운영사업 본공모」를 다음과 같이 공고하오니 참여하고자 하는 지방자치단체는
                            공고문에 따라 신청하시기 바랍니다.
                        </p>
                        <p className="c-date">
                          <strong className="key">진행중인 공고</strong>
                    ...
                    <div className="card-btn">
                      <button type="button" className="krds-btn medium text" title="다문화가족 자녀 언어발달지원서비스"
                        onClick={() => handleToggleLike(index)}><i
                          className={`svg-icon ico-like on-bgcolorgray ${likedItems[index] ? 'on' : ''}`}></i>
                      </button>
                    </div>
                  </li>
                ))}
