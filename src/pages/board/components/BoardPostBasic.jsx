...
const BoardPostBasic = ({ boardDetail, bbsNo, pstNo }) => {
  ...
  const attachmentLabel = useMemo(() => postDetail?.atchFileId || '-', [postDetail]);
  const attachmentUrl = useMemo(() => postDetail?.pstUrlAddr || '#', [postDetail]);

  const moveToList = () => {
    navigate('..');
  };

  const handleAttachmentPreview = (event) => {
    if (!postDetail?.pstUrlAddr) {
      event.preventDefault();
    }
  };

  const handleAttachmentDownload = () => {
    if (!postDetail?.pstUrlAddr) return;
    window.open(postDetail.pstUrlAddr, '_blank', 'noopener,noreferrer');
  };
          ...
          <ul className="box-group-area">
            <li>
              ...
              <div className="btn-wrap">
                ...
                <button type="button" className="krds-btn medium text on-colorblue" onClick={handleAttachmentDownload}>
        ...
        <ul className="post-nav-list mt-40">
          ...
          <li className="post-nav-item prev">
            <Link to="#" className="post-nav-link">
              ...
              <span className="post-nav-title onellipsis-1">-</span>
            </Link>
          </li>
