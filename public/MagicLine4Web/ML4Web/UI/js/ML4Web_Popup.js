				...
				openDialog : function(opt, obj, callback) {
					...
					if(opt.contentKey == "install_cs"){
						...
						if(mode=='main'){
							...
							$('#add_browser_password').on("keydown",function(e){
								e.preventDefault = undefined;
							});
							$('#add_browser_password').focus(function(e){
								$('#add_browser_password').removeAttr("style");
								$('#add_browser_password').removeAttr("readonly");
								$('#form1').css("display","none");
								$('#nppfs-keypad-add_browser_password').css("display","block");
								
								$('#add_browser_password').val("");
							});
							$('#addbrowserKeyboardOn').click(function(e){
							...
							$('#ML_Dialog_mgmt_common').MLjquiWindow({title: dialog_title});
							$('#ML_Dialog_mgmt_common').MLjquiWindow({height:dialogHeight});
							$('#ML_Dialog_mgmt_common').MLjquiWindow({width:398}); // KICS 20231004 인증서 비밀번호 변경 팝업창 넓이 조정
							$('#ML_Dialog_mgmt_common').MLjquiWindow({position:{x: offset1.left + ($(window).width()/2) - 200, y: offset1.top + 120}}); 
							$('#ML_Dialog_mgmt_common').MLjquiWindow({isModal: true,modalOpacity: 0.3,modalZIndex: 99999,modalBackgroundZIndex: 99999}); //KICS 20231011 인증서 비밀번호 변경 팝업창 모달 적용
							$('#ML_Dialog_mgmt_common').MLjquiWindow('open',callback);
							$('#ML_Dialog_mgmt_common').focus();
							return;
						}else{
			...
			function getContentHtml(key) {
				...
				switch(key){
					...
					if(showCert){
						...
						htmlTxt += '		<span class="ML_cert_pw_field f_wh_255">';
						htmlTxt += '			<input type="password" id="input_pw_confirm" name="input_pw_confirm" class="passwd_input" style="ime-mode:disabled;" placeholder="" autocomplete="off" onKeypress="DSDialog.enterKeyEvent(event)" onClick="clickpwd();" data-tk-kbdType="qwerty" npkencrypt="key" data-keypad-type="alpha" data-keypad-theme="default" data-keypad-useyn-type="toggle"/>';
						htmlTxt += '		</span>';
						htmlTxt += '	</div>';
						htmlTxt += '	</div>';
						break;
...
//					console.log( "The public can see me!" );
				...
				openAlert : function(mode, msg, obj){
					...
					if(mode=='main'){
						...
						$('#popup_mgmt_alert').MLjquiWindow({position:{x: ($(window).width()/2) - 185, y: 190}}); //KCIS 20231011 alert 팝업창 사이즈조정
						...
						}); //KICS 20231011 alert창 Modal 옵션 적용
						
						$('#popup_mgmt_alert').MLjquiWindow('open');
						$('#popup_mgmt_alert').focus();	//for WCA, 20200413
					}
				},
