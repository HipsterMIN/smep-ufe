...
function keypadOn(){
	if($("#file_route2").val() != ""){	
		...
		if( ML4WebApi.webConfig.virtualKeyboardType === "RAON" ){
			...
			var add_browser_password_name = $('#add_browser_password').attr("data-keypad-useyn-input");
			$('[name='+add_browser_password_name+']').val("N");
...
/*
 * ML4Web_Main.html 처리하는 이벤트 핸들러 정의
 */
// 가상키보드 활성화
...
// INCA 키보드 아이콘 클릭 이용시 물리키보드 사용할 경우
$('#input_cert_pw').keydown(function(e){
	try{
...
function finance_btnClick(){
	//왼쪽날개 창 동적 생성으로 현재 선택된 탭의 lnb_id 값 가져오기
...
function confirmProcess( data ){
	...
	if( selection && selectionCnt > 0 ){
		for(var i=0 ; i<selectionCnt; i++){
			...
			}else if(ML4WebUI.selectedStorage.current_option.storageName == "new_cloud"){
				storageRawCertIdx = JSON.parse(stgEncCertIdx);
				storageRawCertIdx.pin_pass = cert_pw;
...
function openDriverDialog(id, idx, obj){
	...
	//console.info('id======='+id+(id=="smartcert"));
		...
		if(id=="pfx"){
			...
			//230824 보안토큰 스마트 인증 노출 옵션처리
			if(ML4WebApi.webConfig.cs_smartcert_hidetokenlist){
				for(var idx=0; idx<tokenOptCnt; idx++){
					if(obj.tokenOpt[idx].tokenname.toUpperCase() == 'MOBILE_SMARTCERT'){
						obj.tokenOpt.splice(idx,1);
						break;	
					}
				}
			}
...
//		console.log("flag : " + flag + ", temp : " + temp + ", checkCount : " + checkCount);
	...
	$('#sub_drv_list>li:first').on('keydown', function(e) {
...
function makeSourceData(listObj){
	...
	if(listObj != null && listObj.length > 0){
		for (var i = 0; i < listObj.length; i++) {
			...
			//DN 필터링 기능 추가 20230331
			if(typeof(ML4WebApi.webConfig.dnFilter) != "undefined"){
				var dnFilterObj = ML4WebApi.webConfig.dnFilter; 
				if (dnFilterObj.indexOf(row["Cn"]) !== -1) {
    				data[dataIdx] = row;
    				dataIdx++;
  				}
			} else {
...
function openCertViewDialog(data){
	...
	if ( $("#ML_window_detail").length > 0 ) {
		...
		$("#certDetailContainer").load("../ML4Web/UI/ML4Web_Cert_View.html?random="+magiclineUtil.makeRandomValue() * 99999, function(){
			if(!magiclineUtil.isMobile(magiclineUtil.getOS())){
	...
	DSDialog.openDialog(popOption, btnObj, function(code,jsonObj){
		if(ML4WebApi.getProperty("storageList").indexOf("web_kftc") > -1 || ML4WebApi.getProperty("storageList").indexOf("kftc") > -1 || ML4WebApi.getProperty("storageList").indexOf("web") > -1){
			...
			if( ML4WebApi.webConfig.useVirtualKeyboard && ML4WebApi.webConfig.virtualKeyboardType === "INCA"){
				...
				ML4WebVKey.init(1);
			} else if(ML4WebApi.webConfig.use_keyboard_seucre && ML4WebApi.webConfig.keyboard_secure_type === "Ahnlab") {
				$ASTX2.initE2E($ASTX2_CUST.getCustomerId());
...
function vpad( tag, callback ){
	if(ML4WebApi.getProperty("storageList").indexOf("web_kftc") > -1 || ML4WebApi.getProperty("storageList").indexOf("kftc") > -1 || ML4WebApi.getProperty("storageList").indexOf("web") > -1){
		if( ML4WebApi.webConfig.virtualKeyboardType === "RAON" ){
			...
			var inputId = tag;
			$ASTX2_CUST.getDecryptPassword(inputId, function(result){
				callback(0, result[inputId]);
				$ASTX2.clearE2EText( document.getElementById(inputId) );
			});
		...
		} else if(ML4WebApi.webConfig.useKeyboardSecure) {
...
//핀 가져오기 확인버튼 동작
function importCertToCloud(res,pass){
		
	var b64Cert = res['cert'];
	var b64CertKey = res['key'];
	var kmCert = res['kmcert'];
	var kmPri = res['kmkey'];
	
	var ml4web_crypto_api = ML4WebApi.getCryptoApi();

	var cloudPasswd = pass;
	// 가상키패드 추가

	var bPW = ml4web_crypto_api.HD_api( cloudPasswd );
	
	var targetFields = [];
	var storageRawCertIdx = {};
	storageRawCertIdx.storageName = 'new_cloud';
	storageRawCertIdx.storageOpt = {};
	...
	importCert_cloudDoAction( bPW, targetFields, storageRawCertIdx );
}
function importCert_cloudDoAction( bPW, targetFields, storageRawCertIdx ){
	var resultobj = {};
	var ml4web_crypto_api = ML4WebApi.getCryptoApi();
	...
	ml4web_crypto_api.getcertInfo( storageRawCertIdx.signcert, targetFields, function( code, obj ){
		if(code == 0){
			...
			//console.log(ML4WebUI.selectedStorage.current_option.storageName);
			//console.log(ML4WebUI.selectedStorage.key);
										

			if(ML4WebUI.selectedStorage.key == "new_cloud"){
				targetStorage = "new_cloud";
			}
...
function clickSaveConfirmBtn(){
	if(magiclineUtil.isMobile(magiclineUtil.getOS())){
		if(ML4WebApi.webConfig.use_keyboard_secure) {
			if(ML4WebApi.webConfig.keyboard_secure_type === "Ahnlab") {
				var inputId = "input_cert_pw_save";
				$ASTX2_CUST.getDecryptPassword(inputId, function(result){
					saveCertconfirmProcess(result[inputId]);
					$ASTX2.clearE2EText( document.getElementById(inputId) );
				});
		...
		/*if(magiclineUtil.isMobile(magiclineUtil.getOS())){
			...
			if(selectMedia === "web_kftc" || selectMedia === "web"){
				if( ML4WebApi.webConfig.virtualKeyboardType === "RAON" ){
					...
					var inputId = "input_cert_pw";
					$ASTX2_CUST.getDecryptPassword(inputId, function(result){
						confirmProcess(result[inputId]);
						$ASTX2.clearE2EText( document.getElementById(inputId) );
					});
