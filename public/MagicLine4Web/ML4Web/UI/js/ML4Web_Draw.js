...
//------------------------
	...
	STG_HTML_NCLD      : "<li class='ML_storage_new_cloud'><a href='#' id='.id_key.' stgIdx='.idx.' class='stg_01' title='클라우드'><span id='MSG_TS708'></span></a></li>",
	//STG_HTML_NCLD_INS  : "<li class='ML_storage_new_cloud ins'><a href='#' stgIdx='.idx.' onClick='installCheck(&#34;.mode.&#34;);' class='stg_01' title='클라우드'><span id='MSG_TS708'></span></a></li>",
	//unisign	
	...
	// FINANCE_USER
	STG_HTML_FINANCE_USER  : "<li class='ML_storage_finance_user'><a href='#' id='.id_key.' stgIdx='.idx.' class='stg_01' title='금융인증서(개인)'><span></span></a></li>",
	// FINANCE_CORP
	STG_HTML_FINANCE_CORP  : "<li class='ML_storage_finance_corp'><a href='#' id='.id_key.' stgIdx='.idx.' class='stg_01' title='금융인증서(통합)'><span></span></a></li>",
//--------------------------
 ...
 * */
	...
	loadCSS : function(){
		...
		for( var i = 0; i < ML4WebApi.webConfig.storageList.length; i++){
			if( ML4WebApi.webConfig.storageList[i] === "unisign" ){
				...
				if(!document.getElementById('smartcertnx')){
					/* 인증서 선택창 내 스마트인증 클릭 시 스크립트 읽게 코드 옮김
					var head = document.getElementsByTagName('head')[0];
					var script = document.createElement('script');
					script.type= 'text/javascript';
					script.id = "smartcertnx";
		...
		if(ML4WebApi.getProperty("storageList").indexOf("web_kftc") > -1 || ML4WebApi.getProperty("storageList").indexOf("kftc") > -1 || ML4WebApi.getProperty("storageList").indexOf("web") > -1){
			...
			} else if( ML4WebApi.webConfig.keyboard_secure_type === "Ahnlab" ){
				$.cachedScript(contextPath + "/Ahnlab/astx2/astx2.min.js").done(function () {
					$.cachedScript(contextPath + "/Ahnlab/astx2/astx2_jq.min.js").done(function() {
						$.cachedScript(contextPath + "/Ahnlab/astx2/astx2_custom.js").done(function() {
							$.cachedScript(contextPath + "/Ahnlab/debug.min.js").done(function() {
								$.cachedScript(contextPath + "/Ahnlab/page.js").done(function() {
									checkInstallASTX2(
										function onSuccess() {
											$ASTX2.initE2E($ASTX2_CUST.getCustomerId());
										}
									);
								});
							});
			...
			} else if(ML4WebApi.webConfig.use_keyboard_secure) {
				if( ML4WebApi.webConfig.keyboard_secure_type === "Ahnlab" ){
					$.cachedScript(contextPath + "/Ahnlab/astx2/astx2.min.js").done(function () {
						$.cachedScript(contextPath + "/Ahnlab/astx2/astx2_jq.min.js").done(function() {
							$.cachedScript(contextPath + "/Ahnlab/astx2/astx2_custom.js").done(function() {
								$.cachedScript(contextPath + "/Ahnlab/debug.min.js").done(function() {
		...
		// 스토리지 리스트
		var storageListArray;
		
		if(ML4WebApi.webConfig.storageList_lnb == null || ML4WebApi.webConfig.storageList_lnb == "") {
			//STORAGELIST_LNB = null 값이면 왼쪽 메뉴 날개 div 없애기
			storageListArray = ML4WebApi.webConfig.storageList; //스토리지 리스트 배열
			$(".ML_lnb").css('display', 'none');
		} else {
			if(isMobile){
				storageListArray = ML4WebApi.webConfig.storageList_m;
			} else {
				storageListArray = ML4WebApi.webConfig.storageList_lnb;	//왼쪽날개 리스트 배열
			}
		}
		for( var i = 0; i < storageListArray.length; i++){
			if( storageListArray[i] === "finance" ){
				//통합 금융인증SDK
			...
			} else if (storageListArray[i] === "finance_user" ){
				// 개인용 금융인증SDK
			...
			} else if (storageListArray[i] === "finance_corp" ){
				// 사업자용 금융인증SDK
	...
	_createWindow : function() {
		...
		var closeBtnEnable = ML4WebApi.webConfig.bannerCloseButton;
		...
		$("#ML_window").MLjquiWindow({
			...
			resizable: false,
			position: position,
			showCloseButton:closeBtnEnable,
			closeButtonSize: 32,
			isModal: true,
			modalOpacity: 0.3,
			modalZIndex: 9995,
	...
	initWebMainEvent : function() {
		...
		$('[id^="ML_lnb_"]').click(function(){
			...
			if(lnb_id == "share"){
				...
				$("#ML_dp_01").css('display', 'block');
				$(".ML_storage_box").css('display', 'block');
				// 매직라인 초기화
				$('#temp').val("0");
				$('#flagInit').val("");
				$("#stg_" + ML4WebApi.getProperty("defaultStorage")).click();
			}else if(lnb_id == "new_cloud"){
				$("#Fcert").css('display', 'none'); //금융인증서 문구 제거
				$('#CLUD_01').remove();
				...
				$("#MagicLine").css('display', 'block');
				// 매직라인 초기화
				$("#ML_dp_01").css('display', 'none');
				$(".ML_storage_box").css('display', 'none');

				var stg_id = "new_cloud";
				selectMedia = stg_id;
				...
				//왼쪽날개창 인증서 창 조정
				if(ML4WebApi.webConfig.storageList_lnb == null || ML4WebApi.webConfig.storageList_lnb == ""){
					if(ML4WebApi.webConfig.bannerCloseButton){
				...
				if(lnb_id == "finance"){
					...
					magiclineController.InitFinCertUserAPI(function(){
						var ucpidYN = ML4WebDraw.saveParam.ucpidYN;
						if(typeof ucpidYN == "undefined" || typeof ucpidYN == "N"){
							...
							//ucpidNonce Array to Hex 231019
							var ucpidNonce = ML4WebDraw.saveParam.ucpidNonce;
							var chars = new Uint8Array(ucpidNonce.length * 2);
							var alpha = 'a'.charCodeAt(0) - 10;
							var digit = '0'.charCodeAt(0);
							
							var p = 0;
							for (var i = 0; i < ucpidNonce.length; i++) {
								var nibble = ucpidNonce[i] >>> 4;
								chars[p++] = nibble > 9 ? nibble + alpha : nibble + digit;
								nibble = ucpidNonce[i] & 0xF;
								chars[p++] = nibble > 9 ? nibble + alpha : nibble + digit;    
							}
							
							ML4WebDraw.saveParam.ucpidNonce = String.fromCharCode.apply(null, chars);
							
							ML4WebApi.signAndUCPIDFinCertUserAPI(ML4WebDraw.saveParam, function(code, jsonObj){
								if(code == 0){
									var requestStr = JSON.stringify(jsonObj);
									// - cross domain, send data
									magiclineController.sendResultMessage(requestStr);
 ...
 */
	browser_menu : function() {
		...
		if(ML4WebApi.getProperty('browserNoticeImg')){
			// img
			if(selectMedia == "web" || selectMedia == "web_kftc" || selectMedia == "hdd" || selectMedia == "token" || selectMedia == "mobile" || selectMedia == "smartcertnx"){
				var closeBtnEnable = ML4WebApi.webConfig.bannerCloseButton;
				if(ML4WebApi.webConfig.storageList_lnb == null || ML4WebApi.webConfig.storageList_lnb == "") {
			...
			}else if(selectMedia == "new_cloud"){
				//child += '<div id="browser_manual1" style="width:250px; top:0px;left:526px;  position:absolute;z-index:10002; display:block">';
				//child += '<img id="manual_img" src="UI/images/new_cloud_off.png" style="width:250px; heigth:550px; " tabindex="0" onclick="Storage_API_new_cloud.getCertificateListInfo();" ';
				//child += '<img id="manual_img" src="UI/images/new_cloud_off.png" style="width:250px; heigth:550px; " tabindex="0"  ';
				//child += 'alt="클라우드 이미지">';
				//child += '<img id="manual_close" src="UI/images/btn_hd_close.png" style="width:20px; heigth:20px;position:absolute; left:230px; " alt="브라우저 인증서 사용방법 닫기버튼" title="닫기버튼" tabindex="0" onKeyPress="manualClose(event);">';
				
				//child += '<div id="certList"></div>';								
			}
			child += '</div>';
		}else{
	...
	MakeStorageListDiv : function(obj) {
		...
		if(stgArr != null && stgArr.length > 0){
			...
			for(var i=0; i<stg_length; i++){
				...
				case 'new_cloud':
					//if(!isCsInstall || isCsUpdate){
						//stgHtml += ML4WebDraw.STG_HTML_NCLD_INS.replace(".idx.", idx).replace(".mode.", mode);
					//}else{
						stgHtml += ML4WebDraw.STG_HTML_NCLD.replace(".id_key.", id_key + "new_cloud").replace(".idx.", idx);
					//}
					break;					
				case 'unisign':
				...
				case 'finance':
					...
					break;
				case 'finance_user':
					stgHtml += ML4WebDraw.STG_HTML_FINANCE.replace(".id_key.", id_key + "finance_user").replace(".idx.", idx);
					break;
				case 'finance_corp':
					stgHtml += ML4WebDraw.STG_HTML_FINANCE.replace(".id_key.", id_key + "finance_corp").replace(".idx.", idx);
					break;
				} // end of switch
		...
		if(stgArr != null && stgArr.length > 0){
			...
			for(var i=0; i<stg_length; i++){				
				...
				case 'finance':
					...
					break;
				case 'finance_user':
					stgHtml += ML4WebDraw.STG_HTML_FINANCE_USER.replace(".id_key.", id_key + "finance_user").replace(".idx.", idx);
					break;
				case 'finance_corp':
					stgHtml += ML4WebDraw.STG_HTML_FINANCE_CORP.replace(".id_key.", id_key + "finance_corp").replace(".idx.", idx);
					break;
				default :
					break;
				}
				...
				for(var idx=5; idx<stgArr.length; idx++){
					...
					switch(stgArr[i]){
						...
						case 'cloud':
							stgHtml += ML4WebDraw.STG_HTML_CLD;
						case 'new_cloud':
							stgHtml += ML4WebDraw.STG_HTML_NCLD;							
							break;
						} // end of switch
					} // end of for
 ...
 */
	MakeCertiListDiv : function(listObj) {
				...
				columns: [
					{ text: $.i18n.prop("TS059"), dataField: 'Cn', width: 140, align: 'center',
						cellsRenderer: function (row, column, value, rowData) {
							...
							if(typeof(rowData.new_cloud) != "undefined"){
								if(rowData.new_cloud){
									imgurl = 'UI/images/cert_selected_in_cloud.png';
								}
							}							
		...
		if (listObj!=null && $.isEmptyObject($("#dataTable").MLjquiDataTable('selectRow')) && listObj.length>0){
			//20230210  로컬스토리지에 저장한 cn값으로 리스트 내 인덱스 뽑고
			if (typeof(localStorage) != "undefined" && localStorage.getItem('ML4WebCertCn') != null){
				var cn = magicjs.utf8.decode(magicjs.base64.decode(localStorage.getItem('ML4WebCertCn')));
				var idx = source.localData.findIndex(function(item) {
    				return item.Cn === cn;
  				});
				
				if(idx > -1){
					$("#dataTable").MLjquiDataTable('selectRow', idx);	
				} else { //만료된 인증서
					$("#dataTable").MLjquiDataTable('selectRow', 0);
				}
			} else { //저장한값 없으면 1번째 셀 선택
				$("#dataTable").MLjquiDataTable('selectRow', 0);	
			}
...
// 에러 alert dialog 처리
	errorHandler : function(mode, errCode, obj, callback) {
		...
		//관세청 모바일 UI 인증서 선택창 강제종료
		if(magiclineUtil.isMobile(magiclineUtil.getOS) == true && ML4WebApi.webConfig.showCertDiv == false){
			closeCertDialog('bc');	
		}
	},
