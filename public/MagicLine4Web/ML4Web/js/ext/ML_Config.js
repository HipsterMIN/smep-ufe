...
// SPA 환경 대응: magicline 객체를 $(document).ready 이전에 선언
...
// SPA 환경 대응: magicline.uiapi 초기화 이후에 실행되도록 함수화
function ML_Config_Init() {
	...
	iframe.setAttribute("title",          "dscert");
...
$(document).ready(function(){
	// SPA 환경: magicline.uiapi가 준비될 때까지 대기
	function waitAndInit() {
		if (typeof magicline !== "undefined" && typeof magicline.uiapi === "object" && magicline.uiapi !== "") {
			ML_Config_Init();
		} else {
...
// magicline 객체가 위에서 이미 선언된 경우 재선언하지 않음
if (typeof magicline === "undefined") {
	var magicline = {
		uiapi : "",
		initCallback : "",
...
var magiclineApi = function(){
	...
	function makeUCPIDRequestInfo(userAgreement, ucpidRealName, ucpidGender, ucpidNationalInfo, ucpidBirthDate, ucpidCi, ucpidNonce, ispUrlInfo, callback) {
		...
		//금융인증서 UCPID SDK 호출을 위한 값 셋팅
		param.ispUrlInfo = ispUrlInfo;
		param.ucpidNonce = ucpidNonce;

		param.userAgreeInfo = {};
		param.userAgreeInfo.realName = ucpidRealName;
		param.userAgreeInfo.gender = ucpidGender;
		param.userAgreeInfo.nationalInfo = ucpidNationalInfo;
		param.userAgreeInfo.birthDate = ucpidBirthDate;
		param.userAgreeInfo.ci = ucpidCi;

		param.userAgreement = userAgreement;
		//############## 끝 #################

		//공동인증 UCPID RequestInfo를 만들기 위한 값 셋팅
		...
		var moduleVersion	= { major : 2, minor : 1, build : 0, revision : 0 };		// [코스콤] 테스트 시 약속된 moduleVersion
...
//		if(msg instanceof HTMLFormElement){
	...
	/**
	 * [KICS] 세션 스토리지 사용한 재서명 수행 함수
	 */
	function AutoMakeSignData( msg, sessionId, signOpt, callback ){
		magiclineApi.callback = callback;

		var param = defaultOptions.sign;
		param.signType = "AutoMakeSignData";

		// Param Mapping
		if( msg != null && typeof(msg) != 'undefined' && msg != '' ){
			param.msg = msg;
		}
		if( sessionId!=null && typeof(sessionId) != 'undefined' && sessionId != '' ) {
	...
	function MakeIrosSignData(msg, callback){
		magiclineApi.callback = callback;
		var param = {signType:"MakeSignData",messageType:"",signOpt:{ds_pki_sign:['OPT_USE_CONTNET_INFO','OPT_HASHED_CONTENT'], ds_pki_rsa:'rsa15', ds_pki_hash:'sha256',ds_msg_decode:"true",ds_pki_sign_type:"signeddata"}};
		//if(typeof(msg)=='form')
		if(msg instanceof HTMLFormElement){
			...
			try{
 				var resultB64 = magicjs.base64.decode(msg);
				var md = magicjs.md.create('sha256');
				md.init();
				md.update(resultB64);
				var digest = md.digest();
				param.msg = magicjs.base64.encode(digest);
				...
				if(document.getElementById("addSigner")!= null && typeof document.getElementById("addSigner") != "undefined" ){
					param.signOpt.ds_pki_signdata = document.getElementById("addSigner").value;
	...
	//220707 제큐어 서명데이터 함수 추가
	function MakeXecureSignData(msg, callback){
		 //signOpt makeSignData랑 동일하게
		magiclineApi.callback = callback;
		var param = defaultOptions.sign;
		param.signOpt = {ds_pki_sign:['OPT_USE_CONTNET_INFO','OPT_NO_SIGNED_ATTRIBUTES'], ds_pki_rsa:'rsa15', ds_pki_hash:'sha256',ds_msg_decode:"false"};
		param.signType = "MakeSignData";
		...
		if(magicline.is_ML_Sign_Init){
			//var data = magicline.encodeUtf8Hex(magicline.uiapi.mlUIOpt.msg);
			//var data = param.msg;
			//magicline.ML_sendPostMessageLayer("UI", "MakeIrosSignData", magicline.uiapi.mlUIOpt );

			/*var requestObj = {
				layer: "UI",
				key : "MakeIrosSignData",
				data : param
			};
			*/
	...
	function MakeIrosAddSignData(msg, callback){
		magiclineApi.callback = callback;
		/*var param = defaultOptions.sign;
		param.signOpt = {ds_pki_sign:['OPT_NO_CONTENT','OPT_USE_CONTNET_INFO'], ds_pki_rsa:'rsa15', ds_pki_hash:'sha256',ds_msg_decode:"false"};
		param.signType = "MakeSignData";*/
		var param = {signType:"MakeSignData",messageType:"",signOpt:{ds_pki_sign:['OPT_USE_CONTNET_INFO','OPT_HASHED_CONTENT'], ds_pki_rsa:'rsa15', ds_pki_hash:'sha256',ds_msg_decode:"true",ds_pki_sign_type:"signeddata"}};

		//if(typeof(msg)=='form')
		if(msg instanceof HTMLFormElement){
			param.msg = magicjs.base64.decode(document.getElementById('signData').value);

		}else{
			var firstSignedData = getSignedData(msg);
			//param.msg = magicjs.base64.decode(document.getElementById('signData').value);

			var resultB64 = magicjs.base64.decode(document.getElementById('signData').value);
			var md = magicjs.md.create('sha256');
			md.init();
			md.update(resultB64);
			var digest = md.digest();
		...
		if(typeof msg.decodeOpt != "undefined"){
			param.signOpt.ds_msg_decode = msg.decodeOpt.value;
		}
	...
	function MakeIrosMultiData(msg, callback){
		magiclineApi.callback = callback;

		var param = {signType:"MakeIrosMultiData",messageType:"",signOpt:{ds_pki_sign:['OPT_USE_CONTNET_INFO','OPT_HASHED_CONTENT'], ds_pki_rsa:'rsa15', ds_pki_hash:'sha256',ds_msg_decode:"true",ds_pki_sign_type:"signeddata"}};
...
//		if(msg instanceof HTMLFormElement){
	...
	function getSelectCert( msg , signOpt, callback ){
		...
		param.STORAGELIST_LNB		= defaultOptions.storageList_lnb; // 추가된 코드
	...
	function closeDialog(event){
		try{
			...
			if( typeof(obj.selectStorage) != 'undefined' && obj.selectStorage == 'token' ){
				obj.encMsg = obj.encMsg.sign;
			}
	...
	function ML_funProcInitCheck (callback){
        ...
        var randomValue = parseInt(hexString, 16).toString().substring(0,16);
		var childUrl = mlMainUrl + mlDirPath + childHtml +"?lgUrl="+cpUrl+"&random=" + randomValue;
	...
	return {
		...
		MakeIrosSignData:MakeIrosSignData,
		MakeXecureSignData:MakeXecureSignData,
		MakeIrosAddSignData:MakeIrosAddSignData,
		MakeIrosMultiData:MakeIrosMultiData,
		AutoMakeSignData:AutoMakeSignData
	}
}
...
function getSignedData(sigDataBase64){
	...
	document.reqForm.signData.value= magicjs.base64.encode(pdfFile);
